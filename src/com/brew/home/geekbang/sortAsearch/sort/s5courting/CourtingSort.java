...
public class CourtingSort {
    ...
    public static void main(String[] args) {
        //步骤
        //1. 顺序遍历原始数组A[8], 得到存储A[8]内每个值的元素个数的数组C[6]；C[6]的下标是A[8]的值，C[6]的值是A[8]内每个值的元素个数
        //2. 对C[5] 顺序求和
        //3. 倒序遍历A[8], 逐个拿到当前元素在c[5]中的值，即小于等于当前元素的个数，进而得到当前元素在有序数组中的位置
        int[] sorted = courtingSort(new int[] {2, 5, 3, 0, 2, 3, 0, 3}, 5);
        System.out.println(Arrays.toString(sorted));
    }

    public static int[] courtingSort(int[] origin, int max) {
        //得到：C[6]：   2,0,2,3,0,1
        ...
        //倒序遍历origin
        //最后一个元素3，在tmp[3]中，是7，即小于等于3的有7个，那这个元素在有序数组的未知应该是7-1=6
